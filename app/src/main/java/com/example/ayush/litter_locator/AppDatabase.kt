...
import com.example.ayush.litter_locator.AppDatabase.transdb.C_EMAIL
import com.example.ayush.litter_locator.AppDatabase.transdb.C_FNAME
import com.example.ayush.litter_locator.AppDatabase.transdb.C_INFO
import com.example.ayush.litter_locator.AppDatabase.transdb.C_LOC
import com.example.ayush.litter_locator.AppDatabase.transdb.C_MON
import com.example.ayush.litter_locator.AppDatabase.transdb.C_NAME
import com.example.ayush.litter_locator.AppDatabase.transdb.C_PASSWORD
import com.example.ayush.litter_locator.AppDatabase.transdb.C_TYPE_WASTE
import com.example.ayush.litter_locator.AppDatabase.transdb.C_URI
import com.example.ayush.litter_locator.AppDatabase.transdb.TABLE_LOGIN
import com.example.ayush.litter_locator.AppDatabase.transdb.TABLE_NAME
import com.example.ayush.litter_locator.AppDatabase.transdb.Table_pic
import com.example.ayush.litter_locator.AppDatabase.transdb.abc
...
object transdb{
    ...
    val C_MON = "mno"
    val C_FNAME = "name"
    val C_TYPE_WASTE = "WasteType"
    val C_INFO = "WasteInfo"
    val C_LOC = "location"
    val C_URI = "Puri"
    ...
    override fun onCreate(sqliteDatabase: SQLiteDatabase?) {
        sqliteDatabase?.execSQL("CREATE TABLE "+ transdb.TABLE_NAME +"( "+transdb.C_TYPE_WASTE+" STRING,"+
...
//        _name.equals(na,true)&&_pass.equals(pas)
    ...
    fun qfun(){
        ...
        if (csor.moveToFirst()){
            do{
                var _type = csor.getString(csor.getColumnIndexOrThrow(AppDatabase.transdb.C_TYPE_WASTE))
             ...
             //   Datahelper.car.ttr = _type
                ...
                if(Datahelper.car.wasteHname.equals(_name)){
                    pnts = pnts+10
                }
            }while(csor.moveToNext())
    ...
    fun qpic(){
        val db = this.readableDatabase
        val query = "SELECT * FROM "+ AppDatabase.transdb.Table_pic+" WHERE pname = '${Datahelper.car.wasteHname}'"
        var csor = db.rawQuery(query,null)
    ...
    fun storeData(typeWasteData: String?,infoData: String?,locData: String?,UriData: String?,nameData: String?){
        val db = this.writableDatabase
        var contentValues = ContentValues()
        contentValues.put(transdb.C_TYPE_WASTE,typeWasteData)
